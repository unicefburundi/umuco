...
def analytics(request):
    groups = (
        Report.objects.annotate(colline=F("group__colline"))
        .annotate(commune=F("group__colline__commune"))
        .values("colline", "commune")
        .distinct()
    )
    statistics = []
    for i in groups:
        reports = Report.objects.filter(
            group__colline=i["colline"], group__colline__commune=i["commune"]
        )
        group = i
        ...
        if response_data["text"] != "":
            ...
            if matches:
                ...
                if date_updated.date() > datetime.datetime.today().date():
                    return {
                        "Ok": "False",
                        "info_to_contact": "Itariki ntishobora kuba muri kazoza. SUbira urungike mesaje.",
                        "raba": date_updated,
                    }
                if (date_updated.weekday() + 1) != group.day_of_meeting:
                    return {
                        "Ok": "False",
                        "info_to_contact": "Itariki itegerezwa kuba umusi muhurirako. Subira urungike mesaje yanditse neza.",
                        "raba": date_updated,
                    }
                if (datetime.datetime.today() - date_updated).days > 7:
                    return {
                        "Ok": "Pas",
                        "info_to_contact": "Ntibigishoboka ko murungika raporo. Vugana na C.P.E.S.",
                        "error": (datetime.datetime.today() - date_updated).days,
                    }
                try:
                    ...
                    return {
                        "Ok": "False",
                        "info_to_contact": "Amatara yagurishijwe ntiyanditse neza. Subira urungike raporo neza.",
                        "error": message[1],
                    }
                else:
                ...
                try:
                    ...
                    return {
                        "Ok": "False",
                        "info_to_contact": "Amatara yasharijwe ntiyanditse neza. Subira urungike raporo neza.",
                        "error": message[2],
                    }
                else:
                ...
                try:
                    ...
                    return {
                        "Ok": "False",
                        "info_to_contact": "Amafaranga yaziganijwe ntiyanditse neza. Subira urungike raporo neza.",
                        "error": message[3],
                    }
                else:
                    if not isinstance(message_3, (int)) or message_3 < 0:
                        return {
                            "Ok": "False",
                            "info_to_contact": "Le montant total epargnee n est pas valide. Subira urungike raporo neza.",
                            "error": message_3,
                        }
                try:
                ...
                if matches.group(2):
                    try:
                        ...
                        return {
                            "Ok": "False",
                            "info_to_contact": "Igitigiri c abana mwafashije sico.",
                            "error": message[5],
                        }
                    try:
                        ...
                        return {
                            "Ok": "False",
                            "info_to_contact": "Urwo rudome ntirubaho",
                            "error": message[6],
                        }
                else:
                ...
                repport, created = Report.objects.get_or_create(
                    group=group, date_updated=date_updated
                )
                jsonresponse = {
                    "Ok": "True",
                    "sold_lamps": message_1,
                    "recharged_lamps": message_2,
                    "total_amount": message_3,
                    "pl_amount": message_4,
                    "date": date_updated,
                }
                if created:
                ...
                if matches.group(2):
                    ...
                    for s in list(support_names[1]):
                        support, sp_created = SupportReport.objects.get_or_create(
                            report=repport, kind_of_support=s.upper()
                        )
                        support.childred_supported = message[5]
                        support.save()
                    jsonresponse.update(
                        {"childred_supported": message_5, "support_name": message_6}
                    )
            if len(message) == 3:
                group = PhoneModel.objects.get(
                    number=response_data["phone"]
                ).group.colline
                date_updated = validate_date(message[0])
                return JsonResponse(
                    {"Ok": "True", "group": group.name, "date": date_updated},
                    safe=False,
                )
...
def by_group(request, group=None):
    ...
    return render(
        request,
        "umuco/group_details.html",
        {
            "data": response.content,
            "nawenuze_group": "{1}-{0}".format(
                group.colline.name.title(), group.colline.commune.name.title()
            ),
            "groupe": group,
...
def get_cumulative(request, group=None):
    ...
    if not group:
        reports = Report.objects.values(
            "total_amount", "sold_lamps", "recharged_lamps", "date_updated", "pl_amount"
        ).order_by("date_updated")
    else:
        reports = (
            Report.objects.filter(group=group)
            .values(
                "total_amount",
                "sold_lamps",
                "recharged_lamps",
    ...
    for k in range(max_length)[1:]:
        ...
        cumulative_total_amount.append(
            [date, int(reports[k]["total_amount"] + cumulative_total_amount[k - 1][1])]
        )
        cumulative_recharged.append(
            [date, int(reports[k]["recharged_lamps"] + cumulative_recharged[k - 1][1])]
        )
        cumulative_sold.append(
            [date, int(reports[k]["sold_lamps"] + cumulative_sold[k - 1][1])]
        )
        cumulative_pl_amount.append(
            [date, int(reports[k]["pl_amount"] + cumulative_pl_amount[k - 1][1])]
        )

    return JsonResponse(
        [
            {"name": "Total épargné", "data": cumulative_total_amount},
            {"name": "Lampes rechargées", "data": cumulative_recharged},
            {"name": "Lampes vendues", "data": cumulative_sold},
            {"name": "PL épargné", "data": cumulative_pl_amount},
            {
                "Disposable": cumulative_total_amount[max_length - 1],
                "Charges": cumulative_recharged[max_length - 1],
                "Selling": cumulative_sold[max_length - 1],
            },
        ],
        safe=False,
    )
...
class NaweNuzeDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        reports = Report.objects.filter(group=nawenuzegroup).values(
            "group__colline",
            "group__colline__commune",
            "sold_lamps",
            "recharged_lamps",
            "total_amount",
            "date_updated",
            "group__lamps_in_stock",
            "pl_amount",
        )
...
def add_lamps(request):
    ...
    if response_data["text"] != "":
        ...
        if message[1] not in settings.PASSWORD:
            return {
                "Ok": "Pas",
                "info_to_contact": "Mesaje siyo. Vugana na C.P.E.S.. ",
                "error": message[1],
            }
        if NawenuzeGroup.objects.filter(colline__name=message[2].title()).count() == 0:
            return {
                "Ok": "False",
                "info_to_contact": "Umugwi ntubaho. Vugana na C.P.E.S..",
                "error": message[2],
            }
        try:
            lamps = int(message[3])
            ...
            return {
                "Ok": "False",
                "info_to_contact": "Amatara mwaronse ntiyanditswe neza. Subira urungike raporo neza.",
                "error": message[3],
            }
        else:
        ...
        if date_received.date() > datetime.datetime.today().date():
            return {
                "Ok": "False",
                "info_to_contact": "Itariki ntishobora kuba muri kazoza. Subira urungike raporo neza.",
                "raba": date_received,
            }
        group = NawenuzeGroup.objects.get(colline__name=message[2].title())
        reception, created = Reception.objects.get_or_create(
            group=group, lamps_received=lamps, date_received=date_received
        )
        if created:
    ...
    return {
        "Ok": "True",
        "info_to_contact": "Umugwi ku mutumba {0} (komine {3}) waronse amatara {1} kuwa{2}. Murakoze.".format(
            group, lamps, date_received.strftime("%d-%m-%Y"), group.colline.commune
        ),
        "raba": date_received.strftime("%d-%m-%Y"),
    }
...
class ReportCreate(CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "report:report_list", current_app=self.request.resolver_match.namespace
        )
        ...
        form = NaweNuzeForm(
            {
                ...
                "day_of_meeting": request.POST.get("day_of_meeting"),
                "colline": request.POST.get("colline"),
        ...
        if form.is_valid():
            ...
            if phonemodel_formset.is_valid():
                ...
                messages.add_message(
                    request, messages.SUCCESS, "Saved group and sent SMS"
                )
                return HttpResponseRedirect(
                    reverse("report:group_detail", kwargs={"colline": group.colline})
                )
