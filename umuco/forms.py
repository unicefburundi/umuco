...
from umuco.models import NawenuzeGroup, PhoneModel, Report
...
class NaweNuzeForm(forms.ModelForm):
    ...
    commune = forms.ChoiceField(
        widget=forms.Select(), required=False, choices=([("", "------")])
    )
    colline = forms.ChoiceField(
        widget=forms.Select(), required=False, choices=([("", "------")])
    )
    day_of_meeting = forms.ChoiceField(
        label=_("Day of reporting"),
        widget=forms.Select(),
        required=False,
    ...
    def is_valid(self):
        ...
        try:
            self.cleaned_data["colline"] = Colline.objects.get(
                id=int(self.data.get("colline"))
            )
        except:
...
GroupFormset = inlineformset_factory(
    NawenuzeGroup, PhoneModel, fields=("number",), extra=MAX_PHONENUMBER
)
