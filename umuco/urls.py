...
urlpatterns = patterns('',
	...
	url(r'^new_reporter/$', backend.handel_rapidpro_request, name='new_reporter'),
    url(r'^overview/$', views.get_reports, name='get_reports'),
    url(r'^overview/(?P<name>[0-9a-zA-Z_-]+)$', views.get_reports, name='get_groups_reports'),
    url(r'^export/$', views.download_reports, name='download_reports'),
    url(r'^group/$', include(group_urls ) ),
    url(r'^group/(?P<name>[0-9a-zA-Z_-]+)/reports$', NawenuzeGroupReportList.as_view(), name='groupreport-list'),
    url(r'^group/(?P<name>[0-9a-zA-Z_-]+)$', NawenuzeGroupDetail.as_view(), name='group-detail'),
    url(r'^group/details/(?P<name>[0-9a-zA-Z_-]+)$', views.by_group, name='group_detail'),
)
