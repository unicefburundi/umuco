...
def check_number_of_values(args):
    ...
    if args["message_type"] == "PHONE_REGISTRATION":
        if len(args["text"].split("# ")) < 6:
            ...
            args[
                "info_to_contact"
            ] = "Ikosa. Mwarungitse ibiharuro bike. Subira urungike ibitigiri."
        ...
        if len(args["text"].split("# ")) > 7:
            ...
            args[
                "info_to_contact"
            ] = "Ikosa. Warungitse ibitigiri vyinshi. Subira urungike mesage yawe ikosoye."
...
def check_commune(args):
    ...
    if re.search(expression, the_sent_commune_name):
        ...
        args[
            "info_to_contact"
        ] = "Ikosa. Izina ry ikomine siryo kuko rijamwo indome gusa."
...
def check_colline(args):
    ...
    if re.search(expression, the_sent_colline_name):
        ...
        args[
            "info_to_contact"
        ] = "Ikosa. Izina ry umutumba ritegerezwa kubamwo gusa indome."
...
def check_phone(args):
    ...
    for contact_phone_number in contact_phone_numbers:
        ...
        expression = (
            r"^(\+?(257)?)((62)|(79)|(71)|(76)|(75)|(72)|(61)|(69)|(68))([0-9]{6})$"
        )
        if re.search(expression, contact_phone_number_no_space) is None:
            ...
            args[
                "info_to_contact"
            ] = "Ikosa. I nimero ya terefone mwarungitse ntiyanditse neza."
        elif (
            PhoneModel.objects.filter(
                number="+257" + contact_phone_number_no_space
            ).count()
            > 0
        ):
            args["valide"] = args["valide"] and False
            args[
                "info_to_contact"
            ] = "Ikosa. Iyo nimero ya terefone isanzwe irimwo kuyundi mugwi."
        else:
            args["valide"] = args["valide"] and True
            args[
                "info_to_contact"
            ] = "Inimero ya terefone y umukuru w umugwi yanditswe neza."
...
def record_reporter(args):
    ...
    if colline:
        ...
        args["Ok"] = False
        args["valide"] = False
        args["info_to_contact"] = "Umutumba {0} muri komine {1} ntubaho.".format(
            the_colline, the_commune
        )
    return args
...
def group_confirm(args):
    ...
    for contact_phone_number in contact_phone_numbers:
        ...
        if re.search(expression, contact_phone_number_no_space) is None:
            ...
            args[
                "info_to_contact"
            ] = "Ikosa. Inimero ya terefone mwarungitse ntiyanditse neza. Subiramutangure."
            return args
    ...
    if not temp:
        return {
            "Ok": False,
            "info_to_contact": "Mwarungitse inimero itandukanye niyomwarungitse ubwambere.",
            "valide": False,
        }
    else:
        ...
        if not group:
            group, created = NawenuzeGroup.objects.get_or_create(
                colline=temp.colline,
                day_of_meeting=temp.day_of_meeting,
                lamps_in_stock=temp.lamps_in_stock,
                cost_lamp=temp.cost_lamp,
                cost_recharge=temp.cost_recharge,
            )
            group.save()
        else:
        ...
        args[
            "info_to_contact"
        ] = "Wandikisije umugwi {0} muri komine {1} kugira muzokwame mutanga raporo kuwa {2}.".format(
            the_colline, the_commune, days[int(the_meetting_day)]
        )
        numbers = []
        for the_phone_number in args["text"].split("# "):
            ...
            the_phone_object, created = PhoneModel.objects.get_or_create(
                number=the_phone_number, group=group
            )
            numbers.append(the_phone_number)

        url = "https://app.rapidpro.io/api/v2/broadcasts.json"
        for i in numbers:
            the_message_to_send = "Mwandikishijwe nkuwuzotanga raporo kumugwi {0} muri komine {1}. Turindiriye i raporo misi yose kuwa {2}.".format(
                the_colline, the_commune, days[int(the_meetting_day)]
            )
